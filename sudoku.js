...
function placeable(board, num, row, col) {
    // Here we check the horizontal- ,vertical lines and a 3x3 area to see if we can place the number
    ...
    for (let r = box_row; r < box_row + 3; r++) {
        for (let c = box_col; c < box_col + 3; c++) {
            if (num == board[r][c]) {
                return false;
            }
        }
...
function main() {
    ...
    if (solve(sudoku)) {
        ...
        result.innerHTML = "No Solution! <br> Double check your input.";
    }
}
